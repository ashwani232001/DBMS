...
update student1_details  set sadd= 'mumbai' where ID = 4;
#delete any  specific row
delete from student1_details where ID= 6;
#delete any  specific column
alter table  student1_details Drop column fees;


#alter 
alter table student1_details add Comm_Pct int after fees;
update student1_details  set Comm_Pct=20 where ID=1;
update student1_details  set Comm_Pct=15 where ID=2;
update student1_details  set Comm_Pct=10 where ID=3;
update student1_details  set Comm_Pct=35 where ID=4;
update student1_details  set Comm_Pct=25 where ID=5;
update student1_details  set Comm_Pct=35 where ID=6;
update student1_details  set Comm_Pct=20 where ID=7;
update student1_details  set Comm_Pct=35 where ID=8;
update student1_details  set Comm_Pct=45 where ID=9;

#as
select sname,sadd,fees,fees + 300 as "Increased_fees" from student1_details;
select sname,fees,fees + Comm_Pct + 100 as  'Anual_compensation_fees' from student1_details;
select Comm_Pct from student1_details;
update student1_details  set fees=3000.50 where ID=2;
update student1_details  set fees=3300.50 where ID=3;
update student1_details  set fees=5200.50 where ID=7;
update student1_details  set fees=2500.50 where ID=8;
update student1_details  set fees=4500.50 where ID=6;

#grater than
select sname, fees from student1_details where fees between 2000 and 3000;
select * from student1_details where fees=4500.5 or fees=2000.5 or fees=3300.50;
...
update   student1_details set ID= 11 where fees > 4500.5;  ////
select sname,id from  student1_details where fees>3300.5 and sadd = 'chennai';
